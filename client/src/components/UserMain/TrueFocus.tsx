...
const TrueFocus = ({
  ...
  useEffect(() => {
    ...
    const parentRect = containerRef.current.getBoundingClientRect() as DOMRect;
    const activeRect = wordRefs.current[currentIndex].getBoundingClientRect() as DOMRect;
  ...
  const handleMouseLeave = () => {
    if (manualMode) {
      setCurrentIndex(lastActiveIndex as number);
    }
    setHoverIndex(null);
  };
    ...
    >
      {words.map((word, index) => {
        ...
        return (
          <motion.span
            key={index}
            ref={(el: HTMLSpanElement | null) => (wordRefs.current[index] = el as HTMLSpanElement)}
