...
import { executeCode } from "@/hooks/learningEditor";
import { useRouter, usePathname } from "next/navigation";
import { Play, Send, Loader2, Terminal, AlertCircle, CheckCircle2 } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

const Output = ({ editorRef, language, hideSubmit = false, onRun, className = '' } : { editorRef: any, language: string, hideSubmit: boolean, onRun: () => void, className: string }) => {
  ...
  const runCode = async () => {
    ...
    try {
      ...
      setIsError(true);
      setIsSuccess(false);
      setOutput([error as string || "Unable to run code"]);
    } finally {
      setIsLoading(false);
    }
  };
