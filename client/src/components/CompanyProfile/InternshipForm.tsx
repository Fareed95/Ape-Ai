...
const FormField = ({ label, name, value, onChange, type = "text", icon: Icon, required = true, placeholder, className = "" }: { label: string, name: string, value: string, onChange: any, type: string, icon: any, required: boolean, placeholder: string, className: string }) => (
...
const InternshipForm = () => {
  ...
  const handleSubmit = async (e : any) => {
    ...
    try {
      ...
      const result = await apeService.createInternship(token, internshipData);
      ...
      alert((error as any).message || 'Failed to post internship. Please try again.');
    }
  };
