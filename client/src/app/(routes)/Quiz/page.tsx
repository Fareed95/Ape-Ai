...
function QuizPage() {
  ...
  const [timeLeft, setTimeLeft] = useState(360); // 6 minutes timer
  const timerRef = useRef<any>(null);
  const [isMobile, setIsMobile] = useState(false);
  const [answers, setAnswers] = useState<any[]>([]); // Track answers for each question
  ...
  const handleQuizSubmit = () => {
    ...
    const finalScore = answers.reduce((total : number, answer : string, index : number) => {
      return total + (answer === questions[index].answer ? 1 : 0);
    }, 0);
    
    router.push(`/Quiz/congratulations?score=${finalScore}&total=${questions.length}&topic=${contextinput}`);
  };
...
//   };
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        setLoading(true);
        const response = await fetch(`https://model-ape.crodlin.in/testseries`, {
          method: 'POST',
          headers: {
